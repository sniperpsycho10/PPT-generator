...
export default async function PendingApprovalPage() {
  ...
  const needsProfileCompletion = !dbUser?.departmentId;
      ...
      <div className="card glass" style={{ maxWidth: '500px', width: '100%', textAlign: 'center', padding: '3rem' }}>
        <div style={{ marginBottom: '2rem' }}>
          {needsProfileCompletion ? (
            <UserCheck size={64} color="var(--jspl-blue)" style={{ margin: '0 auto' }} />
          ) : isRejected ? (
        ...
        <h1 style={{ fontSize: '2rem', color: '#333', marginBottom: '1rem' }}>
          {needsProfileCompletion ? 'Complete Your Profile' : isRejected ? 'Access Rejected' : 'Pending Approval'}
        </h1>
        
        {needsProfileCompletion ? (
          <ProfileForm defaultName={dbUser?.name || session.user.name || ""} />
        ) : (
          <>
          ...
          <SignOutButton 
            ...
            style={{ 
              ...
              fontSize: '1rem',
              textDecoration: 'none',
              marginTop: needsProfileCompletion ? '1rem' : '0',
